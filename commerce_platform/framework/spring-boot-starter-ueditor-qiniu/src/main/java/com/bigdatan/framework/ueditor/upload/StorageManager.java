...
import com.bigdatan.framework.ueditor.define.FileType;
...
public class StorageManager {
	...
	public static State saveBinaryFile(byte[] data, String path) {
		...
		try {
			...
			Response response = new UploadManager(new Configuration()).put(data, key, uploadToken);
	...
	private static State saveTmpFile(File tmpFile, String path) {
		...
		try {
			...
			String uploadToken = Auth.create(accessKey, secretKey).uploadToken(bucket);
			Response response = new UploadManager(new Configuration()).put(tmpFile, key, uploadToken);
			if (response.statusCode == 200) {
				state = new BaseState(true);
